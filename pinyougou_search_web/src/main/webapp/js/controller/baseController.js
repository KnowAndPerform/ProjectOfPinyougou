...
app.controller("baseController",function($scope){
    ...
    //判断结果集里边是不是有attrubuteName了
    $scope.exitAttributeName=function(specItems,key,name){
        //遍历结果集:[{"attributeName":"网络","attributeValue":["移动3G"]}]
        for(var i=0;i<specItems.length;i++){
            if(specItems[i][key]==name){
                //返回当前的对象:{"attributeName":"网络","attributeValue":["移动3G"]}
                return specItems[i];
            }
        }
        return null;
